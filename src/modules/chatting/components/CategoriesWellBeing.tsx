...
import { WriteDiary } from '@src/modules/home/components/well-being/WriteDiary'
import { ItemCategory } from '@src/modules/home/components/well-being/ItemCategory'

const swiperVerticalStyles = `
  .swiper-vertical-custom .swiper-wrapper {
    flex-direction: column !important;
    align-items: stretch !important;
  }
  .swiper-vertical-custom .swiper-slide {
...
interface CategoriesWellBeingProps {
  musicData: AudioItem[]
  podcastData: AudioItem[]
  yogaData: AudioItem[]
  storyData: AudioItem[]
}

export function CategoriesWellBeing({ musicData, podcastData, yogaData, storyData }: CategoriesWellBeingProps) {
  ...
  useEffect(() => {
    const fetching = async () => {
      try {
        setIsLoading(true)
        let response: AudioItem[] = []
        if (selected === 'music') {
          response = musicData
        } else if (selected === 'podcast') {
          response = podcastData
        ...
        const mappedItems: MappedAudioItem[] = (response || []).map((item) => ({
          title: item.title || 'Không có tiêu đề',
          time: '-- phút', // Placeholder, will be updated async
          image: getImageWithFallback(item.image, item.uri),
          isPlaying: false,
          divRef: null,
          uri: item.uri
        }))
  ...
  return (
    <div className='flex flex-col gap-4' id='categories-well-being'>
      {/* Inject CSS styles */}
      <style dangerouslySetInnerHTML={{ __html: swiperVerticalStyles }} />
      <SlideUp className='flex justify-center mb-4 mt-4'>
        <div className='text-center text-black text-2xl font-semibold md:text-4xl md:font-medium md:leading-normal md:w-[70%]'>
          Dưới đây là danh sách âm nhạc & podcast & tài nguyên miễn phí của mentacare cho bạn
        </div>
      </SlideUp>
            ...
            <div className='h-[350px] md:h-[400px] overflow-hidden w-[90%]'>
              {isLoading ? (
                ...
                <Swiper
                  ...
                  // virtual={true}
                  // modules={[Virtual]}
                  className='h-full swiper-vertical-custom'
                >
                  {list.map((item, index) => (
